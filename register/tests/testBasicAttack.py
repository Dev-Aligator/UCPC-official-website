...
from register.models import UcpcUser
...
class UserTest(TestCase):
    def setUp(self):
        user_a = UcpcUser(email='user@user.com')
        user_a_pw = '123456'
    ...
    def test_user_exists(self):
        user_count = UcpcUser.objects.all().count()
        self.assertEqual(user_count, 1)
        self.assertNotEqual(user_count, 0)
    
    def test_user_password(self):
        user_a = UcpcUser.objects.get(email='user@user.com')
        self.assertTrue(user_a.check_password(self.user_a_pw))
    
    def test_login_url(self):
        login_url = reverse('register:login') 
        data = {"password": self.user_a_pw, "email":"user@user.com"}  
        response = self.client.post(login_url, data, follow=True)
        status_code = response.status_code
        self.assertEqual(status_code, 200)
    
    def test_invalid_request(self):
        self.client.login(email=self.user_a.email, password="invalid_pw")
